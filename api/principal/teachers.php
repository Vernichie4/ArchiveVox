        ...
        if ($action === 'list') {
            ...
            $stmt = $pdo->prepare('
                SELECT 
                    ...
                    /* FIXED: Renamed to last_assessed so JS can read it */
                    (SELECT MAX(ar.assessed_at) 
                    FROM assessment_result ar 
                    INNER JOIN reading_activity ra ON ar.activity_id = ra.activity_id 
                    WHERE ra.student_id = s.student_id) AS last_assessed,
                    
                    /* FIXED: Added reading_level so badges colorize properly */
    ...
    elseif ($method === 'POST') {
        ...
        if ($action === 'create') {
            ...
            try {
                ...
                // Insert class (grade + section + school year) for this teacher
                $stmt = $pdo->prepare('
